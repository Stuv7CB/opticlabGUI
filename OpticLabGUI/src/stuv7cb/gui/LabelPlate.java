...
class LabelPlate extends LabelObject
{
	...
	void setDimension()
	{
		...
		Point end=new Point();
		d.setSize(Math.abs(length*Math.cos(Math.PI/2.0-angle))+Math.abs(span*Math.cos(angle))+10, Math.abs(length*Math.sin(Math.PI/2.0-angle))+Math.abs(span*Math.sin(angle))+10);
